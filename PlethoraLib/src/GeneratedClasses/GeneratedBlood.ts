...
export interface IBloodPropertiesValues {
  "sugar level": number
  "insulin level": number
  "glucagon level": number
}

export abstract class GeneratedBlood extends AbstractUiEntity {
  constructor(id: number, propertyValues: PropertyValuesDictionary) {
    super("Blood", id, propertyValues, [])
  }
  ...
  protected onPropertyUpdate(propertyName: string, newValue: any, oldValue: any, uiData: UiData): void {
    ...
    switch (propertyName) {
      ...
      case "insulin level": {
        this.onInsulinLevelUpdate(newValue, oldValue, uiData)
        return
      }

      case "glucagon level": {
        this.onGlucagonLevelUpdate(newValue, oldValue, uiData)
        return
      }
  ...
  public getInsulinLevel(): number {
    return this.getPropertyValue("insulin level") as number
  }

  public setInsulinLevel(value: number, uiData: UiData = null): void {
    this.setPropertyValue("insulin level", value, uiData, true)
  }

  protected abstract onInsulinLevelUpdate(newValue: number, oldValue: number, uiData: UiData): void

  public getGlucagonLevel(): number {
    return this.getPropertyValue("glucagon level") as number
  }

  public setGlucagonLevel(value: number, uiData: UiData = null): void {
    this.setPropertyValue("glucagon level", value, uiData, true)
  }

  protected abstract onGlucagonLevelUpdate(newValue: number, oldValue: number, uiData: UiData): void
  ...
  public static createRequest(
    propertyValues: Partial<IBloodPropertiesValues>,
    quantity: number,
    uiData: UiData = null
  ): void {

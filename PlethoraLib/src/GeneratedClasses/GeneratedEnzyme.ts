...
export interface IEnzymePropertiesValues {
  State: DataTypeState
}

export abstract class GeneratedEnzyme extends AbstractUiEntity {
  constructor(id: number, propertyValues: PropertyValuesDictionary) {
    super("Enzyme", id, propertyValues, [])
  }
  ...
  protected onPropertyUpdate(propertyName: string, newValue: any, oldValue: any, uiData: UiData): void {
    ...
    switch (propertyName) {
      case "State": {
        this.onStateUpdate(newValue, oldValue, uiData)
        return
      }
  ...
  public getState(): DataTypeState {
    return this.getPropertyValue("State") as DataTypeState
  }

  public setState(value: DataTypeState, uiData: UiData = null): void {
    this.setPropertyValue("State", value, uiData, true)
  }

  protected abstract onStateUpdate(newValue: DataTypeState, oldValue: DataTypeState, uiData: UiData): void
  ...
  protected onInteraction(
    ...
    switch (interactionName) {
      ...
      case "Deconstruct": {
        this.onDeconstruct(true, reactorEntity, uiData) // activator
        ;(reactorEntity as any).onDeconstruct(false, this, uiData) // reactor
        return
      }
  ...
  public applyDeconstruct(reactorId: number, uiData: UiData = null): void {
    this.applyInteraction(reactorId, "Deconstruct", {}, uiData, true)
  }
  protected abstract onDeconstruct(isActivator: boolean, otherEntity: AbstractEntity, uiData: UiData): void

  public static createRequest(
    propertyValues: Partial<IEnzymePropertiesValues>,
    quantity: number,
    uiData: UiData = null
  ): void {
